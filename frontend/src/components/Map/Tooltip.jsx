...
function TooltipIMQ({ p }) {
  ...
  return (
    <>
      <p style={{ fontWeight: 600, fontSize: 13, color: 'var(--text)', marginBottom: 2 }}>{p.iris_nom}</p>
      <p style={{ fontSize: 11, color: 'var(--text-2)', marginBottom: 10 }}>arr. {p.arrondissement}</p>
      ...
      <div style={{ marginBottom: 12 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
          <span style={{ fontSize: 11, color: 'var(--text-2)' }}>Score IMQ</span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px 12px' }}>
        {[
          ...
          <div key={item.label}>
            <p style={{ fontSize: 10, color: 'var(--text-3)', marginBottom: 1 }}>{item.label}</p>
            <p style={{ fontSize: 12, fontWeight: 500, color: 'var(--text)' }}>{item.value}</p>
          </div>
        ))}
      </div>
    </>
...
function TooltipITR({ p }) {
  ...
  return (
    <>
      <p style={{ fontWeight: 600, fontSize: 13, color: 'var(--text)', marginBottom: 2 }}>{p.nom_voie}</p>
      <p style={{ fontSize: 11, color: 'var(--text-2)', marginBottom: 10 }}>arr. {p.arrondissement}</p>
      <div style={{ display: 'inline-block', background: color + '22', border: `1px solid ${color}55`, borderRadius: 20, padding: '2px 10px', marginBottom: 10 }}>
      ...
      <div style={{ marginBottom: 12 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
          <span style={{ fontSize: 11, color: 'var(--text-2)' }}>Score ITR</span>
...
function TooltipGeneric({ indicator, p }) {
  ...
  return (
    <>
      ...
      <div style={{ marginBottom: 12 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
          <span style={{ fontSize: 11, color: 'var(--text-2)' }}>{cfg.scoreLabel}</span>
...
export default function Tooltip({ indicator, feature, x, y }) {
  ...
  return (
    <div style={{
      ...
      background: 'rgba(253,250,246,0.97)', border: '1px solid var(--border)',
