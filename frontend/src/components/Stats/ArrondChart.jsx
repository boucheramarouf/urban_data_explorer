...
function CustomTooltip({ active, payload, indicator }) {
  ...
  return (
    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 6, padding: '6px 10px', boxShadow: 'var(--shadow-sm)' }}>
      <p style={{ fontSize: 12, color: 'var(--text)', fontWeight: 600 }}>{d.arrondissement}e arr.</p>
      <p style={{ fontSize: 11, color: 'var(--text-2)' }}>Score médian : <b style={{ color }}>{score}</b></p>
      <p style={{ fontSize: 11, color: 'var(--text-2)' }}>{isIMQ ? `${d.nb_iris} IRIS` : `${d.nb_rues} rues`}</p>
    </div>
  )
}
...
export default function ArrondChart({ indicator, data }) {
  ...
  return (
    <div>
      <p style={{ fontSize: 11, color: 'var(--text-2)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 10 }}>
        Classement par arrondissement
      </p>
      <ResponsiveContainer width="100%" height={320}>
        <BarChart data={sorted} layout="vertical" margin={{ left: 0, right: 0, top: 0, bottom: 0 }}>
          <XAxis type="number" domain={[0, 100]} hide />
          <YAxis type="category" dataKey="arrondissement" tick={{ fontSize: 10, fill: 'var(--text-3)' }} tickFormatter={v => `${v}e`} width={28} />
          <Tooltip content={<CustomTooltip indicator={indicator} />} cursor={{ fill: 'rgba(29,78,107,0.05)' }} />
