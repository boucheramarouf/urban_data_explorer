...
export default function StatsPanel({ indicator, stats }) {
  if (!stats) return (
    <div style={{ padding: 16, color: 'var(--text-3)', fontSize: 12, textAlign: 'center' }}>Chargement des stats...</div>
  )
    ...
    <div>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        {Object.entries(stats.distribution_label || {}).map(([label, count]) => {
          ...
          const color = isIMQ ? (IMQ_COLOR[label] || 'var(--accent)') : (LABEL_COLOR[label] || 'var(--text-3)')
          return (
            <div key={label} style={{ marginBottom: 8 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
                <span style={{ fontSize: 11, color: 'var(--text)' }}>{label}</span>
                <span style={{ fontSize: 11, color: 'var(--text-2)' }}>{count} · {pct}%</span>
              </div>
