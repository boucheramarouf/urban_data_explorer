    ...
    <div style={{ position: 'relative', marginBottom: 16 }}>
      <div style={{
        ...
        <input
          ...
          style={{ background: 'none', border: 'none', outline: 'none', color: '#f0f2ff', fontSize: 13, width: '100%' }}
        />
        {query && (
      ...
      {focused && results.length > 0 && (
        <div style={{
          ...
          background: '#1a1d27', border: '1px solid #2e3348', borderTop: 'none',
          borderRadius: '0 0 8px 8px', zIndex: 100, maxHeight: 240, overflowY: 'auto',
        }}>
          {results.map((item, i) => (
            <div key={i} onMouseDown={() => handleSelect(item)}
              style={{ padding: '8px 12px', cursor: 'pointer', borderBottom: '1px solid #2e3348', transition: 'background 0.15s' }}
              onMouseEnter={e => e.currentTarget.style.background = '#21253a'}
              onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
            >
              <p style={{ fontSize: 12, fontWeight: 500, color: '#f0f2ff' }}>
