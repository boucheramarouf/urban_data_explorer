...
const CommerceToggle = ({ value, onChange }) => (
  <div style={{ marginBottom: 12 }}>
    <p style={{ fontSize: 11, color: 'var(--text-3)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>
      Présence commerce
    </p>
    <div style={{ display: 'flex', gap: 6 }}>
      {[
        ...
        <button
          ...
          style={{
            flex: 1,
            background: String(value) === String(opt.value) ? 'rgba(29,78,107,0.10)' : 'transparent',
            border: `1px solid ${String(value) === String(opt.value) ? 'var(--accent)' : 'var(--border)'}`,
            borderRadius: 6,
            color: String(value) === String(opt.value) ? 'var(--accent)' : 'var(--text-2)',
    ...
    <div style={{ padding: '0 0 4px' }}>
      ...
      {hasFilter && (
        <button onClick={() => onChange({})} style={{
          ...
          borderRadius: 7, color: 'var(--text-2)', fontSize: 12, padding: '6px',
